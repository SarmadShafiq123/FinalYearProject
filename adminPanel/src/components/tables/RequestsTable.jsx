...
const RequestsTable = ({ requests, onApprove, onReject, onStatusChange, showActions = true }) => {
  return (
    <div className="overflow-x-auto">
      <table className="w-full">
        <thead>
          <tr>
            {["Name", "Email", "Plan", "Amount", "Payment", "Storage", "Date", "Status", ...(showActions ? ["Actions"] : [])].map((h) => (
              <th
                key={h}
        ...
        <tbody>
          {requests.map((r) => (
            <tr key={r._id} className="border-b border-zinc-800/50 hover:bg-zinc-800/20 transition-colors">
              <td className="text-sm text-zinc-300 py-3 pr-4">{r.name}</td>
              <td className="text-sm text-zinc-300 py-3 pr-4 font-mono text-xs">{r.email}</td>
              ...
              <td className="py-3 pr-4">
                <span className={`text-xs px-2 py-0.5 rounded-md font-medium capitalize ${paymentStatusBadge(r.paymentStatus || "pending")}`}>
                  {r.paymentStatus || "pending"}
                </span>
              </td>
              <td className="text-sm text-zinc-400 py-3 pr-4">{formatBytes(r.storageBytes)}</td>
              <td className="text-sm text-zinc-400 py-3 pr-4">{formatDate(r.createdAt)}</td>
              <td className="py-3 pr-4">
                {showActions ? (
                  <select
                    value={r.status}
          ...
          {requests.length === 0 && (
            <tr>
              <td colSpan={showActions ? 9 : 8} className="text-center text-sm text-zinc-500 py-8">
                No requests found.
              </td>
            </tr>
          )}
